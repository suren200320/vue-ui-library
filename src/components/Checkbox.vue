  ...
  <div class="checkbox" @click="handleCheck">
    ...
    <div :class="['checkbox-custom', isChecked && 'checked', size, side]">
      <img
        ...
        v-if="modelValue"
        class="checkbox-custom-checkmark"
      />
    </div>
    <label v-if="label" :class="['checkbox-label', side]">{{ label }}</label>
  </div>
</template>
...
const isChecked = computed(() =>
  Array.isArray(props.modelValue) && typeof props.value === 'string'
...
const handleCheck = () => {
  if (Array.isArray(props.modelValue) && typeof props.value === 'string') {
    const updated = props.modelValue.includes(props.value)
